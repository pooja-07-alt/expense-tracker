...
function renderBudget(total) {
    ...
    if (percent < 60) {
        bar.style.background = 'linear-gradient(90deg, #7c6ff7, #a78bfa)';
    ...
    list.innerHTML = expenses.map((e, i) => `
        <li>
            <div class="expense-info">
                ...
                <span class="expense-date">${e.date ? new Date(e.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : 'No date'}</span>
            </div>
...
function renderChart(expenses) {
    ...
    const colors = ['#7c6ff7', '#f472b6', '#34d399', '#fbbf24', '#f87171', '#60a5fa'];
